...
function computedFileHex(filepath) {
  return new Promise((resolve, reject) => {
    ...
    stream.on('end', function() {
      ...
      resolve(hex + '[' + path.basename(filepath) + ']');
    });
  });
}

exports.generateSrcHash = (srcPath, ignoreArr = []) => {
  ignoreArr = ignoreArr.map(ignore => {
    return ignore.replace(/[\\/]+/g, '/');
  });
  return globby('**/**', {
    noext: true,
    onlyFiles: true,
...
exports.computedMD5 = computedMD5;
